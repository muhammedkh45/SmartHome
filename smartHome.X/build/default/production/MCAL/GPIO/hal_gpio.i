...
uint8_t gpio_pin_direction_intialize(const pin_config_t *_pin_config){
    uint8_t ret = 1;
    if((0) == _pin_config || _pin_config->pin > 8 -1){
        ret = 0;
    }
    else{
...
# 54 "MCAL/GPIO/hal_gpio.c"
uint8_t gpio_pin_get_direction_status(const pin_config_t *_pin_config, direction_t *direction_status){
    uint8_t ret = 1;
    if((0) == _pin_config || (0) == direction_status || _pin_config->pin > 8 -1){
        ret = 0;
    }
    else{
...
# 104 "MCAL/GPIO/hal_gpio.c"
uint8_t gpio_pin_read_logic(const pin_config_t *_pin_config, logic_t *logic){
    uint8_t ret = 1;
    if((0) == _pin_config || (0) == logic || _pin_config->pin > 8 -1){
        ret = 0;
    }
    else{
...
uint8_t gpio_port_get_direction_status(port_index_t port, uint8_t *direction_status){
    uint8_t ret = 1;
    if(((0) == direction_status) && (port > 5 -1)){
        ret = 0;
    }
    else{
